...
<body>
    ...
    <div id="confirmation-modal_delete" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full h-full">
        <div class="relative p-4 w-full max-w-md h-full mx-auto flex justify-center items-center">
            <div class="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
                ...
                <p class="mb-4 text-gray-500">Are you sure you want to unset this ride?</p>
    ...
    // Retrieve data from the (reserve) table
    $reserve_sql = "SELECT r.*, rd.driver_id, d.name AS name
    FROM reservation r
    JOIN ride rd ON r.ride_id = rd.ride_id
    JOIN driver d ON rd.driver_id = d.driver_id
    WHERE r.student_id = $student_id AND rd.status = 'undone'";
    ...
    <h1 class="text-3xl text-center p-4 my-8 text-gray-800">My Current Reservation</h1>
    <div class="flex justify-center">
        ...
        if ($reserve_result !== null && $reserve_result->num_rows > 0) {
            while ($row = $reserve_result->fetch_assoc()) {
                ...
                echo '<label class="text-gray-800 text-xs uppercase font-semibold">Driver Name</label>';
                ...
                echo '</div>';
                echo '<div class="px-6 py-4">';
                echo '<a href="actions/chat.php?id=' . $row['driver_id'] . '" class="btn btn-sm mr-4 text-white bg-blue-600 hover:bg-blue-400">Open Chat</a>';
